//! Shared agentd header rendering.
//!
//! Inspired heavily by the work in Vite+ by @Brooooooklyn.
//!
...
const HEADER_SUFFIX: &str = " - agent multiplexer";
const HEADER_FALLBACK: &str = "agentd - agent multiplexer";

const RESET_FG: &str = "\x1b[39m";
const DEFAULT_VIOLET: Rgb = Rgb(185, 131, 255);
const DEFAULT_CYAN: Rgb = Rgb(95, 251, 255);
...
fn get_header_colors() -> &'static HeaderColors {
    HEADER_COLORS.get_or_init(|| {
        ...
        let blue = palette_color(&palette, ANSI_BLUE_INDEX).unwrap_or(DEFAULT_CYAN);
        let magenta = palette_color(&palette, ANSI_MAGENTA_INDEX).unwrap_or(DEFAULT_VIOLET);
...
fn render_header_variant(
    ...
    let agentd = format!("{}agentd{RESET_FG}", fg_rgb(primary));
    let suffix = colorize(HEADER_SUFFIX, suffix_colors);
    format!("{}{}", bold(&agentd, prefix_bold), bold(&suffix, suffix_bold))
}

/// Render the shared agentd header string with terminal-aware coloring.
#[must_use]
pub fn agentd_header() -> String {
    if !should_colorize() || !supports_true_color() {
        return HEADER_FALLBACK.to_string();
    }
